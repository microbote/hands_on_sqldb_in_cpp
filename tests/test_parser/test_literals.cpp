...
const ASTNode *first_value(const ASTNode *insert) {
  ...
  const auto *rows = (const ASTNodeList *)data->values->data;
  const ASTNode *first_row = rows->head;
  if (first_row == nullptr || first_row->type != NODE_LIST) {
    return first_row; // 手工构造的扁平 AST（兼容）
  }
  const auto *row = (const ASTNodeList *)first_row->data;
  return row->head;
}
...
TEST(Literals, IntegerLiteralsKeepFullInt64Range) {
  ...
  CHECK_EQ(number_value(parse_insert("9223372036854775807").get()), INT64_MAX);
}
...
TEST(Literals, StringEscapesDoubleSingleQuote) {
  ...
  CHECK_EQ(std::string(string_value(parse_insert("'it''s'").get())),
           std::string("it's"));
  CHECK_EQ(std::string(string_value(parse_insert("''").get())),
           std::string(""));
