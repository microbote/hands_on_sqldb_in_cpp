...
TEST(SqlStatements, IsNullPredicates) {
  ...
  CHECK(
      parse_ok("SELECT * FROM users WHERE age IS NULL AND name IS NOT NULL;"));
}
...
TEST(SqlStatements, ComplexStatements) {
  ...
  CHECK(parse_ok("SELECT * FROM products WHERE (price > 100 OR price < 10) AND "
                 "(category = 'Electronics' OR category = 'Books');"));
  CHECK(parse_ok("SELECT id, name, age, email FROM users "
...
TEST(SqlStatements, CApiPathWorks) {
  // main.cpp 直接使用 C API：yy_scan_string / yyparse / g_parsed_ast /
  // reset_parser
...
TEST(SqlStatements, ParseMultiReturnsOneResultPerStatement) {
  parser::Parser parser;
  auto results = parser.parse_multi("SELECT * FROM users; \n"
...
TEST(SqlStatements, LineAndBlockCommentsAreSkipped) {
  parser::Parser parser;

  // 行注释（含语句末尾、独占一行、行内三种位置）
  CHECK(parser.parse("-- leading comment\nSELECT * FROM users;").success);
  CHECK(parser.parse("SELECT * FROM users; -- trailing").success);
  CHECK(parser.parse("SELECT\n  id -- 行内注释\nFROM users;").success);

  // 块注释（含跨行）
  CHECK(parser.parse("/* block */ SELECT * FROM users;").success);
  CHECK(parser.parse("SELECT * /* 跨\n行 */ FROM users;").success);

  // 注释不改变 AST
  auto with_comment = parser.parse("-- c\nSELECT id FROM users;");
  auto plain = parser.parse("SELECT id FROM users;");
  CHECK(with_comment.success && plain.success);
  if (with_comment.success && plain.success) {
    CHECK(with_comment.ast->type == plain.ast->type);
  }
}
