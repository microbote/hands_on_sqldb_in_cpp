...
class KeyRange {
 ...
 public:
  ...
  static KeyRange empty() {
    return KeyRange(RangeType::EMPTY);
  }
  ...
  bool has_start() const {
    return is_bounded() && !start_.is_null();
  }
  bool has_end() const {
    return is_bounded() && !end_.is_null();
  }
  ...
  // 结构合法性：区间定义是否自洽
  bool is_valid() const {
    if (is_empty() || is_all()) return true;
    if (is_bounded()) {
      if (has_start() && has_end() && start_ >= end_) return false;
      return true;
    }
    return false;
  }
  ...
  // 是否为单点区间（仅对整数类型有效）
  bool is_point() const {
    if (!is_bounded() || !has_start() || !has_end()) return false;
    if (!start_.is_int() || !end_.is_int()) return false;
    return (end_.int_val() - start_.int_val()) == 1;
  }

  // 获取单点值（仅当 is_point() 为 true）
  sql::Value point_value() const {
    if (is_point()) return start_;
    return sql::Value();
  }

  // 估算区间元素个数（仅对整数类型有效）
  std::optional<int64_t> size() const {
    if (!is_bounded() || !has_start() || !has_end()) return std::nullopt;
    if (!start_.is_int() || !end_.is_int()) return std::nullopt;
  ...
  bool contains(const sql::Value& val) const {
    ...
    if (is_all()) return true;
    if (is_bounded()) {
      if (has_start() && val < start_) return false;
      if (has_end() && val >= end_) return false;
      return true;
    }
    return false;
  }
  ...
  bool overlaps(const KeyRange& other) const {
    ...
    // 两个有界区间
    if (has_start() && other.has_end() && start_ >= other.end_) return false;
    if (has_end() && other.has_start() && end_ <= other.start_) return false;
    return true;
  }
  ...
  bool adjacent(const KeyRange& other) const {
    if (is_empty() || other.is_empty()) return false;
    if (is_all() || other.is_all()) return false;
    if (has_end() && other.has_start() && end_ == other.start_) return true;
    if (has_start() && other.has_end() && start_ == other.end_) return true;
    return false;
  }
  ...
  bool covers(const KeyRange& other) const {
    ...
    if (other.is_all()) return false;
    // 两者都是 BOUNDED
    if (has_start() && !other.has_start()) return false;
    if (has_end() && !other.has_end()) return false;
    if (has_start() && other.has_start() && start_ > other.start_) return false;
    if (has_end() && other.has_end() && end_ < other.end_) return false;
    return true;
  }
  ...
  std::vector<KeyRange> subtract(const KeyRange& other) const {
    ...
    // 有界区间 - 其他区间
    // 如果其他区间完全包含当前区间，返回空
    if (other.covers(*this)) return {empty()};
    ...
    } else if (!other.has_end() || (has_end() && other.end_ >= end_)) {
  ...
  std::vector<KeyRange> complement() const {
    return to_complement(*this);
  }

      // 补集（仅对全集或有限区间有效，可能产生两个区间）
      std::vector<KeyRange> to_complement(const KeyRange& r)
          const {if (r.is_empty()) return {
            all()
          };
  if (r.is_all()) return {empty()};
  std::vector<KeyRange> result;
  if (r.is_bounded()) {
    // (-∞, start) 和 (end, +∞)
    KeyRange left = to(r.start_);
    KeyRange right = from(r.end_);
    if (left.is_nonempty()) result.push_back(left);
    if (right.is_nonempty()) result.push_back(right);
  }
    return result;
  }
  ...
  // 转换为 KV 引擎的 KeyRange
  // ============================================================
  inline std::optional<kv::KeyRange> to_kv_range() const {
    kv::KeyRange kvr;
    auto & k = *this;
    if (k.is_empty()) {
      // 空集无法表示，返回无效范围
