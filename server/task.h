...
//
// 为什么只有一个返回类型 `Task<void>`：服务器里所有协程的"结果"都通过
...
//   `ServiceThread::submit()`（见 service.h）—— 把活交给 parse
//   服务线程/写线程，
//      完成后由对方 `Loop::post()` 回本线程再恢复。
// 执行 SQL 本身是同步的（`Session::execute_parsed` 一跑到底），不在这里让出。
//
// 线程模型：一个 Task 只在**一个** Loop 上跑；跨线程只允许 `Loop::post()` 投递
// "继续执行"的动作。因此"同一时刻只有一个线程碰这个 session"是构造性成立的。
#pragma once
